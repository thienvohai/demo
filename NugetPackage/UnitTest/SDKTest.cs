using Microsoft.Extensions.DependencyInjection;
using EmailService.SDK;
using EmailService.Domain;
using System.Reflection;
using Microsoft.Extensions.Configuration;

namespace UnitTest;

public class SDKTest
{
    private IEmailServiceSDKClient GetEmailSDKClient()
    {
        var configBuilder = new ConfigurationBuilder().AddUserSecrets(Assembly.GetExecutingAssembly());
        var config = configBuilder.Build();
        var serviceCollection = new ServiceCollection();
        serviceCollection.AddEmailClient(option =>
        {
            option.EmailServiceEndpoint = config.GetSection("EmailClientSDKOption:EmailServiceEndpoint").Value;
            option.From = config.GetSection("EmailClientSDKOption:From").Value;
            option.SenderName = config.GetSection("EmailClientSDKOption:SenderName").Value;
            option.AuthEndpoint = config.GetSection("EmailClientSDKOption:AuthEndpoint").Value;
            option.ClientId = config.GetSection("EmailClientSDKOption:ClientId").Value;
            option.ClientSecret = config.GetSection("EmailClientSDKOption:ClientSecret").Value;
        });
        var serviceProvider = serviceCollection.BuildServiceProvider();
        return serviceProvider.GetService<IEmailServiceSDKClient>()!;
    }
    [Fact]
    public async Task TestHealthzSend()
    {
        using var emailclient = GetEmailSDKClient();
        var healhzResponse = await emailclient!.HealthzAsync();

        Assert.True(healhzResponse.IsSuccess);
    }

    [Fact]
    public async Task TestEmailSend()
    {
        using var emailclient = GetEmailSDKClient();
        var emailMessage = new EmailMessage()
        {
            To = new List<string> { "Leo.Vo@gmail.com" },
            Cc = new List<string> { "Clara.Nguyen@gmail.com", "young.Nguyen@gmail.com" },
            Bcc = new List<string> { "Stellantt.Nguyen@gmail.com" },
            Body = "<h1>Hi Clara aka Voi Ban Don</h1><br><img src=\"cid:inlineidabc123\"></img>",
            Subject = $"Test Email Service At {DateTime.UtcNow}",
            IsBodyHtml = true,
            Attachments = new List<AttachmentItem>{
                        new AttachmentItem
                        {
                            FileName = "test.png",
                            ContentType = "image/png ",
                            IsInlineDisposition = true,
                            Base64Content = "",
                            ContentId = "inlineidabc123"
                        }
                    }
        };

        var sendEmailResponse = await emailclient.SendEmailAsync(emailMessage);

        Assert.True(sendEmailResponse.IsSuccess);
    }

    [Fact]
    public async Task TestEmailSendFiltered()
    {
        using var emailclient = GetEmailSDKClient();
        var emailMessage = new EmailMessage()
        {
            To = new List<string> { "Young.Nguyen@gmail.com" },
            Cc = new List<string> { "Clara.Nguyen@gmail.com", "young.Nguyen@gmail.com" },
            Bcc = new List<string> { "Stellantt.Nguyen@gmail.com" },
            Body = "<h1>Hi Clara aka Voi Ban Don</h1><br><img src=\"cid:inlineidabc123\"></img>",
            Subject = $"Test Email Service At {DateTime.UtcNow}",
            IsBodyHtml = true,
            Attachments = new List<AttachmentItem>{
                        new AttachmentItem
                        {
                            FileName = "test.png",
                            ContentType = "image/png ",
                            IsInlineDisposition = true,
                            Base64Content = "",
                            ContentId = "inlineidabc123"
                        }
                    }
        };

        var sendEmailResponse = await emailclient.SendEmailAsync(emailMessage);

        Assert.True(sendEmailResponse.IsSuccess);
    }

    [Fact]
    public async Task TestGetEmailDetail()
    {
        using var emailclient = GetEmailSDKClient();
        var sendEmailResponse = await emailclient.GetEmailDetailAsync(Guid.NewGuid());

        Assert.True(sendEmailResponse.IsSuccess);
    }
}
